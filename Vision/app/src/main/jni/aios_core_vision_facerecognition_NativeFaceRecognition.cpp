...
static int DELAY_IMAGE_ADD_FACEBATCH = 80;
...
JNIEXPORT jstring JNICALL Java_aios_core_vision_facerecognition_NativeFaceRecognition_update
  ...
  // if faceTracking isEmpty => return ""
    ...
    if( parseSuccessful && jsonFTResult.size() == 0)
...
//    }
    ...
    if(isDebug) LOG("Final Result: LAST CHECK FRRESULT");
    for (int i = 0 ; i < jsonFRResult.size(); i++){
          face_id = jsonFRResult[i]["face_id"].asString();
          face_name = jsonFRResult[i]["face_name"].asString();
          if(isDebug) LOG("Final Result: face_id %s, face_name %s", face_id.c_str(), face_name.c_str());

          Json::Value jsonObject;
          jsonObject["time"] = 0;
          jsonObject["face_id"] = face_id;
          jsonObject["face_name"] = face_name;
          jsonObject["position"][0] = 0;
          jsonObject["position"][1] = 0;
          jsonObject["position"][2] = 0;
          jsonObject["position"][3] = 0;

          jsonResult[jsonResult.size()] = jsonObject;
    }
